...
def initialize_parameters():
    ...
    x1_encoder_h5 = tf.get_variable("x1_encoder_h5", [128,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b5 = tf.get_variable("x1_encoder_b5", [128], initializer = tf.zeros_initializer())
    x2_encoder_h5 = tf.get_variable("x2_encoder_h5", [128,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b5 = tf.get_variable("x2_encoder_b5", [128], initializer = tf.zeros_initializer())

    x1_encoder_h6 = tf.get_variable("x1_encoder_h6", [128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b6 = tf.get_variable("x1_encoder_b6", [64], initializer = tf.zeros_initializer())
    x2_encoder_h6 = tf.get_variable("x2_encoder_h6", [128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b6 = tf.get_variable("x2_encoder_b6", [64], initializer = tf.zeros_initializer())
      
    joint_encoder_h1 = tf.get_variable("joint_encoder_h1", [128,15], initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_Late_FC/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_Late_FC/model.ckpt")
