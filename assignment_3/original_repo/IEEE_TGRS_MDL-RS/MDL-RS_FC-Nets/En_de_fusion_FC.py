...
def initialize_parameters():
    ...
    x1_encoder_h1 = tf.get_variable("x1_encoder_h1", [144,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b1 = tf.get_variable("x1_encoder_b1", [16], initializer = tf.zeros_initializer())
    x2_encoder_h1 = tf.get_variable("x2_encoder_h1", [21,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b1 = tf.get_variable("x2_encoder_b1", [16], initializer = tf.zeros_initializer())
    
    x1_encoder_h2 = tf.get_variable("x1_encoder_h2", [16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b2 = tf.get_variable("x1_encoder_b2", [32], initializer = tf.zeros_initializer())
    x2_encoder_h2 = tf.get_variable("x2_encoder_h2", [16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b2 = tf.get_variable("x2_encoder_b2", [32], initializer = tf.zeros_initializer())

    x1_encoder_h3 = tf.get_variable("x1_encoder_h3", [32,64], initializer =tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b3 = tf.get_variable("x1_encoder_b3", [64], initializer = tf.zeros_initializer())
    x2_encoder_h3 = tf.get_variable("x2_encoder_h3", [32,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b3 = tf.get_variable("x2_encoder_b3", [64], initializer = tf.zeros_initializer()) 

    x1_encoder_h4 = tf.get_variable("x1_encoder_h4", [64,128], initializer =tf.keras.initializers.VarianceScaling(seed=1))
    x1_encoder_b4 = tf.get_variable("x1_encoder_b4", [128], initializer = tf.zeros_initializer())
    x2_encoder_h4 = tf.get_variable("x2_encoder_h4", [64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_encoder_b4 = tf.get_variable("x2_encoder_b4", [128], initializer = tf.zeros_initializer()) 
    
    joint_encoder_h1 = tf.get_variable("joint_encoder_h1", [256,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    joint_encoder_b1 = tf.get_variable("joint_encoder_b1", [128], initializer = tf.zeros_initializer())
    joint_encoder_h2 = tf.get_variable("joint_encoder_h2", [128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    joint_encoder_b2 = tf.get_variable("joint_encoder_b2", [64], initializer = tf.zeros_initializer())
    joint_encoder_h3 = tf.get_variable("joint_encoder_h3", [64,15], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    joint_encoder_b3 = tf.get_variable("joint_encoder_b3", [15], initializer = tf.zeros_initializer())       

    x1_decoder_h1 = tf.get_variable("x1_decoder_h1", [128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_decoder_b1 = tf.get_variable("x1_decoder_b1", [64], initializer = tf.zeros_initializer())
    x2_decoder_h1 = tf.get_variable("x2_decoder_h1", [128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_decoder_b1 = tf.get_variable("x2_decoder_b1", [64], initializer = tf.zeros_initializer())
    
    x1_decoder_h2 = tf.get_variable("x1_decoder_h2", [64,32], initializer =tf.keras.initializers.VarianceScaling(seed=1))
    x1_decoder_b2 = tf.get_variable("x1_decoder_b2", [32], initializer = tf.zeros_initializer())
    x2_decoder_h2 = tf.get_variable("x2_decoder_h2", [64,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_decoder_b2 = tf.get_variable("x2_decoder_b2", [32], initializer = tf.zeros_initializer())
    
    x1_decoder_h3 = tf.get_variable("x1_decoder_h3", [32,16], initializer =tf.keras.initializers.VarianceScaling(seed=1))
    x1_decoder_b3 = tf.get_variable("x1_decoder_b3", [16], initializer = tf.zeros_initializer())
    x2_decoder_h3 = tf.get_variable("x2_decoder_h3", [32,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_decoder_b3 = tf.get_variable("x2_decoder_b3", [16], initializer = tf.zeros_initializer())

    x1_decoder_h4 = tf.get_variable("x1_decoder_h4", [16,144], initializer =tf.keras.initializers.VarianceScaling(seed=1))
    x1_decoder_b4 = tf.get_variable("x1_decoder_b4", [144], initializer = tf.zeros_initializer())
    x2_decoder_h4 = tf.get_variable("x2_decoder_h4", [16,21], initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x2_train_set, x1_train_set_full, x2_train_set_full, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_EnDe_FC/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_EnDe_FC/model.ckpt")
