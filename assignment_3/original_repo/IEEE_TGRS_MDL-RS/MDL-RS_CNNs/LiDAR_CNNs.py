...
def initialize_parameters():
    ...
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [3,3,21,16], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    ...
    x1_conv_w5 = tf.get_variable("x1_conv_w5", [1,1,128,15], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_LiDAR_CNN/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_LiDAR_CNN/model.ckpt")
