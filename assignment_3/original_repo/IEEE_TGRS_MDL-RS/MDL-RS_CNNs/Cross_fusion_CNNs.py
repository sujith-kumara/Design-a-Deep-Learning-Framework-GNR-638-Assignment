...
def initialize_parameters():
    ...
    joint_encoder_h1 = tf.get_variable("joint_encoder_h1", [1,1,256,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    joint_encoder_b1 = tf.get_variable("joint_encoder_b1", [128], initializer = tf.zeros_initializer())   

    joint_encoder_h2 = tf.get_variable("joint_encoder_h2", [1,1,128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    joint_encoder_b2 = tf.get_variable("joint_encoder_b2", [64], initializer = tf.zeros_initializer())   

    joint_encoder_h3 = tf.get_variable("joint_encoder_h3", [1,1,64,15], initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_CNN_Cross/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_CNN_Cross/model.ckpt")
