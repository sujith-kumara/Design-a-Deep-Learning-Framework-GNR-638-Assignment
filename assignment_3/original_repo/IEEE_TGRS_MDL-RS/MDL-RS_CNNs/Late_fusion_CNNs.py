...
def initialize_parameters():
    ...
    x1_conv_w31 = tf.get_variable("x1_conv_w31", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b31 = tf.get_variable("x1_conv_b31", [128], initializer = tf.zeros_initializer())
    x2_conv_w31 = tf.get_variable("x2_conv_w31", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b31 = tf.get_variable("x2_conv_b31", [128], initializer = tf.zeros_initializer())
    
    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,128,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b4 = tf.get_variable("x1_conv_b4", [128], initializer = tf.zeros_initializer())
    x2_conv_w4 = tf.get_variable("x2_conv_w4", [1,1,128,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b4 = tf.get_variable("x2_conv_b4", [128], initializer = tf.zeros_initializer())
         
    x1_conv_w5 = tf.get_variable("x1_conv_w5", [1,1,128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b5 = tf.get_variable("x1_conv_b5", [64], initializer = tf.zeros_initializer())   
    x2_conv_w5 = tf.get_variable("x2_conv_w5", [1,1,128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b5 = tf.get_variable("x2_conv_b5", [64], initializer = tf.zeros_initializer())   
    
    x1_conv_w6 = tf.get_variable("x1_conv_w6", [1,1,128,15], initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x2_train_set, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL, 
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_CNN_Late/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_CNN_Late/model.ckpt")
