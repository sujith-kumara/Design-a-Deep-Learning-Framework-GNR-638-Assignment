...
def initialize_parameters():
    ...
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [3,3,144,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b1 = tf.get_variable("x1_conv_b1", [16], initializer = tf.zeros_initializer())
    x2_conv_w1 = tf.get_variable("x2_conv_w1", [3,3,21,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b1 = tf.get_variable("x2_conv_b1", [16], initializer = tf.zeros_initializer())
    
    x1_conv_w2 = tf.get_variable("x1_conv_w2", [1,1,16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b2 = tf.get_variable("x1_conv_b2", [32], initializer = tf.zeros_initializer())
    x2_conv_w2 = tf.get_variable("x2_conv_w2", [1,1,16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b2 = tf.get_variable("x2_conv_b2", [32], initializer = tf.zeros_initializer())
    
    x1_conv_w3 = tf.get_variable("x1_conv_w3", [3,3,32,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b3 = tf.get_variable("x1_conv_b3", [64], initializer = tf.zeros_initializer())
    x2_conv_w3 = tf.get_variable("x2_conv_w3", [3,3,32,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b3 = tf.get_variable("x2_conv_b3", [64], initializer = tf.zeros_initializer())

    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b4 = tf.get_variable("x1_conv_b4", [128], initializer = tf.zeros_initializer())
    x2_conv_w4 = tf.get_variable("x2_conv_w4", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_conv_b4 = tf.get_variable("x2_conv_b4", [128], initializer = tf.zeros_initializer())
         
    x1_conv_w5 = tf.get_variable("x1_conv_w5", [1,1,256,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b5 = tf.get_variable("x1_conv_b5", [128], initializer = tf.zeros_initializer())   

    x1_conv_w6 = tf.get_variable("x1_conv_w6", [1,1,128,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b6 = tf.get_variable("x1_conv_b6", [64], initializer = tf.zeros_initializer())   

    x1_conv_w7 = tf.get_variable("x1_conv_w7", [1,1,64,15], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b7 = tf.get_variable("x1_conv_b7", [15], initializer = tf.zeros_initializer())   
   
    x1_deconv_w1 = tf.get_variable("x1_deconv_w1", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_deconv_b1 = tf.get_variable("x1_deconv_b1", [64], initializer = tf.zeros_initializer())
    x2_deconv_w1 = tf.get_variable("x2_deconv_w1", [1,1,64,128], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_deconv_b1 = tf.get_variable("x2_deconv_b1", [64], initializer = tf.zeros_initializer())
    
    x1_deconv_w2 = tf.get_variable("x1_deconv_w2", [3,3,32,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_deconv_b2 = tf.get_variable("x1_deconv_b2", [32], initializer = tf.zeros_initializer())
    x2_deconv_w2 = tf.get_variable("x2_deconv_w2", [3,3,32,64], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_deconv_b2 = tf.get_variable("x2_deconv_b2", [32], initializer = tf.zeros_initializer())
    
    x1_deconv_w3 = tf.get_variable("x1_deconv_w3", [3,3,16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_deconv_b3 = tf.get_variable("x1_deconv_b3", [16], initializer = tf.zeros_initializer())
    x2_deconv_w3 = tf.get_variable("x2_deconv_w3", [3,3,16,32], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x2_deconv_b3 = tf.get_variable("x2_deconv_b3", [16], initializer = tf.zeros_initializer())

    x1_deconv_w4 = tf.get_variable("x1_deconv_w4", [3,3,144,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_deconv_b4 = tf.get_variable("x1_deconv_b4", [144], initializer = tf.zeros_initializer())
    x2_deconv_w4 = tf.get_variable("x2_deconv_w4", [3,3,21,16], initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x2_train_set, x1_train_set_full, x2_train_set_full, x1_test_set, x2_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_CNN_EnDe/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_CNN_EnDe/model.ckpt")
