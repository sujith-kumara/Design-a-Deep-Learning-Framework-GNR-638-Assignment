...
def initialize_parameters():
    ...
    x1_conv_w1 = tf.get_variable("x1_conv_w1", [3,3,165,16], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b1 = tf.get_variable("x1_conv_b1", [16], initializer = tf.zeros_initializer())
  
    x1_conv_w2 = tf.get_variable("x1_conv_w2", [1,1,16,32], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b2 = tf.get_variable("x1_conv_b2", [32], initializer = tf.zeros_initializer())
  
    x1_conv_w3 = tf.get_variable("x1_conv_w3", [3,3,32,64], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b3 = tf.get_variable("x1_conv_b3", [64], initializer = tf.zeros_initializer())

    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,64,128], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b4 = tf.get_variable("x1_conv_b4", [128], initializer = tf.zeros_initializer())
         
    x1_conv_w5 = tf.get_variable("x1_conv_w5", [1,1,128,128], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b5 = tf.get_variable("x1_conv_b5", [128], initializer = tf.zeros_initializer())   

    x1_conv_w6 = tf.get_variable("x1_conv_w6", [1,1,128,64], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
    x1_conv_b6 = tf.get_variable("x1_conv_b6", [64], initializer = tf.zeros_initializer())    
 
    x1_conv_w7 = tf.get_variable("x1_conv_w7", [1,1,64,15], dtype=tf.float32, initializer = tf.keras.initializers.VarianceScaling(seed=1))
...
def train_mynetwork(x1_train_set, x1_test_set, y_train_set, y_test_set, MODEL,
    ...
    with tf.Session() as sess:
        ...
        print("save model")
        os.makedirs(os.path.dirname("./save_CNN_Early/model.ckpt"), exist_ok=True)
        save_path = saver.save(sess,"./save_CNN_Early/model.ckpt")
